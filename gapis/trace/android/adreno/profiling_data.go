...
func ProcessProfilingData(ctx context.Context, processor *perfetto.Processor, desc *device.GpuCounterDescriptor, handleMapping *map[uint64][]service.VulkanHandleMappingItem, submissionIds *map[api.CommandSubmissionKey][]uint64) (*service.ProfilingData, error) {
...
func processGpuSlices(ctx context.Context, processor *perfetto.Processor, handleMapping *map[uint64][]service.VulkanHandleMappingItem, subCommandIndicesMap *map[api.CommandSubmissionKey][]uint64) (*service.ProfilingData_GpuSlices, error) {
			...
			if _, ok := subCommandGroupMap[key]; !ok {
				if indices, ok := (*subCommandIndicesMap)[key]; ok {
					group := &service.ProfilingData_GpuSlices_Group{
						Id:   int32(len(groups)),
						Link: &path.Command{Indices: indices},
					}
	...
	for i := uint64(0); i < numSliceRows; i++ {
		...
		if names[i] == "Surface" {
			names[i] = fmt.Sprintf("%v", groups[groupIds[i]].Link)
		}
