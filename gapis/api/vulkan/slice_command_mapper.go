		...
		if uint64(id) >= t.offset {
			...
			for i := uint32(0); i < submitInfoCount; i++ {
				...
				for j := uint32(0); j < cmdBufferCount; j++ {
					commandIndex := []uint64{uint64(id) - t.offset, uint64(i), uint64(j), 0}
...
func newSliceCommandMapper(offset uint64, subCommandIndicesMap *map[api.CommandSubmissionKey][]uint64) *sliceCommandMapper {
	return &sliceCommandMapper{offset: offset, subCommandIndicesMap: subCommandIndicesMap, submissionCount: 0}
}
